    ...
    // import OrderModel from './models/Error.mjs';
    ...
    const standardizeData = async (data, WORK_ENV) => {
        let standarData = {}, CUST_NUM = 828, MAIL_LIST = ['', '']
        if (WORK_ENV === 'PROD') {
            CUST_NUM = 828
            MAIL_LIST = ['', '']
        }
    ...
    const createDetailBody = async (OV, items, CUST_NUM, OrderComment, AUTH, xApiKey, url, warehouse) => {
        ...
        for (const [index, item] of items.entries()) {
            ...
            const { sku, name, quantity, price , partnum} = item
            console.log('item==',item)
            objItem = { partnum, price, name, quantity }

            // /api/run/sp/parnum?parnum=SUPREMACY 30&company=HMMX
            const data_115 = await (await fetch(`${URLEpcior}/api/run/sp/parnum?parnum=${partnum}&company=HMMX`)).json();
            const {ok,data} = data_115 
            let _quantity = parseInt(quantity)
            let _price = parseInt(price)
            let PartNum = (ok == true && data.length>0)? data[0].partnum: objItem.partnum
            let PartDescription =( ok == true && data.length) ? data[0].partdescription: objItem.name
    ...
    const createDetail = async (orderID, bodyDetail) => {
        try {
            ...
            const realizeBody = await createRealizeBody(OrderNum, OrderLine, quantity, parseInt(CUSTOM_NUM), "vallejo", "mfgsys")
